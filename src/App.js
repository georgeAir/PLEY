...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      apiKey:
        "ue2_GLAE9FpEp0NX5hhSw_6qzFoN-MlrnL1Sm7BJUnuixUy4u3MnLp_FqUxqpbyMTzqkqLujFbQRfOgFZUP19cxZo5da-uDeU3OnQJ1KhmPZg1LZssAXl494sszyYXYx",
      restaurants: [],
      bars: [],
      favorites: [],
      favoriteToBeEdited: {},
  ...
  handleSubmitEdit = async (e) => {
    ...
    e.preventDefault();
    const url =
      this.state.baseURL + "/favorites/" + this.state.favoriteToBeEdited;
    try {
      ...
      if (response.status === 200) {
        ...
        const findIndex = this.state.favorites.findIndex(
          (favorite) => favorite._id === updatedFavorite.data._id
        );
        const copyFavorites = [...this.state.favorites];
              ...
              return (
                <Card className="barCard" style={{ width: "18rem" }}>
                  ...
                  <Card.Body>
                    ...
                    <Card.Text>Price: {bar.price}</Card.Text>
                    <Card.Text>Rating: {bar.rating}</Card.Text>
                    <Card.Text>Phone Number: {bar.phone}</Card.Text>
                    <Card.Text>{bar.location.display_address}</Card.Text>
                  </Card.Body>
                </Card>
              );
            })}
            ...
            {this.state.favorites.map((favorite, i) => {
              return (
                <Card className="foodCard" style={{ background: "orange" }}>
                  ...
                  />
                  <Card.Body>
                    <Card.Title className="favoriteName">
                      {favorite.name}
                    </Card.Title>
                    ...
                    <Card.Text>
                      ...
                      <br />
                      <button onClick={() => this.addLike(favorite)}>
                        Like:❤️‍
                      </button>
                    </Card.Text>
                    <Card.Text>Phone: {favorite.phone}</Card.Text>
                    <Card.Text>{favorite.display_address}</Card.Text>
                    <button onClick={() => this.showEditForm(favorite._id)}>
                      Edit
                    </button>{" "}
                    <br />
                    <br />
                    <button onClick={() => this.deleteFavorite(favorite._id)}>
                      Delete:❌
                    </button>
                  </Card.Body>
                </Card>
